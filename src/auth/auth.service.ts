...
export class AuthService {
  ...
  async socialSignin(
    ...
    try {
      ...
      const { accessToken: token, refreshToken } = await this.issueTokens(user);
      ...
      const hasActiveSubscription =
        await this.subscriptionRepository.hasActiveSubscription(
          user._id.toString(),
        );
  ...
  async userSignUp(body: UserSignUpDTO, req: Request): Promise<ApiResponse> {
  ...
  async loginUser(body: UserSignInDTO, req: Request): Promise<ApiResponse> {
  ...
  async refreshToken(body: RefreshJwtDto): Promise<ApiResponse> {
    ...
    if (tokenData?._id) {
      ...
      if (!refreshTokenData) throw new BadRequestException('Invalid token!');
      ...
      if (!user?._id) throw new BadRequestException('User not found!');
  ...
  async forgotPassword(body: EmailDTO): Promise<ApiResponse> {
    ...
    await this.redisHelper.set(codeKey, verificationCode.toString(), 'EX', 300);
  ...
  async verifyEmail(body: CustomerEmailVerificationDTO): Promise<ApiResponse> {
  ...
  async resetPassword(body: CustomerResetPasswordDTO): Promise<ApiResponse> {
